...
def read_pm_dataset():
    # Raw data filenames
    data_dir = '/media/disk1/jychoi/data/seoul/china'
    data_filenames = ['airquality.csv']
    ...
    for data_filename in data_filenames:
        ...
        # station information
        df_geo = pd.read_csv('station.csv')
        df = pd.merge(df, df_geo[['station_id', 'latitude', 'longitude']], on='station_id')

        # time expresion change
        df['time'] = (df['time'].str[0:4] + df['time'].str[5:7] +
                      df['time'].str[8:10] + df['time'].str[11:13]).astype(int)

        # select station enough data
        df2 = df.groupby('station_id').size().to_frame('size')
        y = df2[df2['size'] > 7000].index.values
        df = df[df['station_id'].isin(y)]
        ...
        df = df.stack(['station_id'])
        df = df.reset_index()

        df = df.rename(columns={'station_id': '측정소코드', 'time': '측정일시',
                                'PM25_Concentration': 'PM25', 'PM10_Concentration': 'PM10'})
        dfs.append(df)
...
def split_data_to_train_eval(df_features, df_labels):
    idx = pd.IndexSlice
    dates_train = idx[:2015013124]
    dates_eval = idx[2015020101:]
